...
public class LobbyManager : MonoBehaviour
{
    ...
    {
        get => Mathf.Clamp(PlayerPrefs.GetInt(PrefArenaIndex, 0), 0, Mathf.Max(0, (matchesPerArena?.Length ?? 1) - 1));
    ...
    public void ShowBattle()
    {
        ...
        {
            if (!arenaBoards[i]) continue;
            if (i==0) PopOpen(arenaBoards[i]); else arenaBoards[i].SetActive(false);
        }
